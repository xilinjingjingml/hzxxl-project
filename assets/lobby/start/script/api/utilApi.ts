...
export namespace Util {
    ...
    export function SetVibrationVolume(vibration: number) {
        cc.sys.localStorage.setItem("vibrationVolume", vibration.toString());
    }
    ...
    export function GetVibrationVolume(): number {
        let vibration = cc.sys.localStorage.getItem("vibrationVolume")
        vibration && (vibration = Number(vibration))
        if (undefined === vibration || null === vibration || "" === vibration || Number.isNaN(vibration))
            vibration = 1
        return vibration
    }
