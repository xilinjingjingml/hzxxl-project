...
export namespace UIMgr {
    export function OpenUI(bundle: cc.AssetManager.Bundle | string, name?: string | UIParam | Function, param?: UIParam | Function, callback?: Function) {
        ...
        if (typeof bundle === "string") {
            name = bundle
            bundle = DataMgr.data.Bundle
        }

        if (!bundle) {
            DataMgr.data.Bundle = cc.assetManager.getBundle("lobbySub")
            bundle = DataMgr.data.Bundle
        }

        if (!bundle) {
            console.log("OpenUI bunlde is null")
            PlatformApi.loadBundleLobbySub((err, bundle)=>{
                if(err){
                    callback && callback({ err: "bundle is null" })
                }else{
                    UIMgr.OpenUI(bundle, name, param, callback)
                }                    
            });
            return
        }
        ...
        if (typeof name !== "string") {
            console.warn("OpenUI name is not string")
            callback && callback({ err: "name type err" })
            return
        }

        console.log("UIMgr.OpenUI " + name)
        igs.platform.trackUserAction("UIMgr.OpenUI " + name)
    ...
    export function CloseUI(scene: string | cc.Node) {
        ...
        if (scene instanceof cc.Node && scene.isValid) {
            ...
            EventMgr.dispatchEvent(Constants.EVENT_DEFINE.UI_CLOSE, { sceneName: scene["name"] })
	        igs.platform.trackUserAction("UIMgr.CloseUI " + scene["name"])
    ...
    function enter(src: cc.Prefab, name: string, param: UIParam, callback: Function) {
        ...
        igs.platform.trackUserAction("UIMgr.enter " + name)
