...
import { AccountSrv } from "../system/AccountSrv"
...
export namespace PluginMgr {
    ...
    export function login(loginInfo: ILoginInfo) {
        console.log("[PluginManager.login] loginInfo=", JSON.stringify(loginInfo))
        console.log("[PluginManager.login] _pluginProxy=", _pluginProxy)
        console.log("[PluginManager.login] Helper.isNative()=", Helper.isNative())
        console.log("[PluginManager.login] !loginInfo.sessionType=", !loginInfo.sessionType)
        if (Helper.isNative() && !loginInfo.sessionType) {            
            AccountSrv._loginGuest({})
            // if (!UIMgr.FindUI("component/Personal/SessionPop")) {
            //     PlatformApi.loadBundleLobbySub(()=>{
            //         UIMgr.OpenUI("component/SessionPop/SessionPop", { single: true, parent: cc.director.getScene() })
            //     })
            // }            
    ...
    export function showBanner(param: any, callback?: Function) {
        if (/*!Player.GetSelf().newbie &&*/ _pluginProxy && _pluginProxy.showAds && (!_showBanner || _pauseBanner)) {
            _showBanner = true
            _callBanner = true
            setTimeout(() => {
                _callBanner = false
            ...
            _bannerAdInfo = _bannerAds[0]
            param = param || {}
            param.adArea = param.adArea || 0
            param.createNew = param.createNew || false
            for(let v of _bannerAds){
                if(v.adArea == param.adArea){
                    _bannerAdInfo = v
                }
            }
            if (_bannerAdInfo) {
                _bannerAdInfo.createNew = param.createNew
                console.log("showBanner " + _bannerAdInfo.adId + " : " + JSON.stringify(_bannerAdInfo))
                _loadPlugin(_bannerAdInfo?.adPlugin, EPluginType.kPluginAds)
                _pluginProxy.showAds(JSON.stringify(_bannerAdInfo))
            }
        }
    }
    ...
    export function navigateToMiniGame(appId: string, extraData: any = null) {
        _pluginProxy.navigateToMiniGame && _pluginProxy.navigateToMiniGame(appId, extraData)
    }
