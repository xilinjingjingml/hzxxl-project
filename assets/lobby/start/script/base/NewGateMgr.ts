...
export namespace NewGateMgr {
    ...
    export function login(host: string, port?: number, path?: string, sender?: Function) {
        ...
        if (/*bundle && */cc.sys.platform === cc.sys.ANDROID) {
            connent("assets/resources/cacert.pem")
            // cc.resources.load("cacert", (err, pem) => {
            //     console.log(pem)
            //     if (err) {
            //         cc.error(err)
            //         connent(null)
    ...
    function onOpen(i, e) {
        console.log("===NewGateMgr on socket open " + Date.now())
        callbacks.clear()
    ...
    function onClose(i, e) {
        console.log("===NewGateMgr on socket close " + Date.now())
        heartbeat_timer && clearInterval(heartbeat_timer)
    ...
    function onError(error) {
        console.log("===NewGateMgr on socket error")
        console.log(error)
    ...
    export function ready() {
        console.log("===NewGateMgr ready=", _ready)
        // if (!_ready) {
        //     login(_host, _port)
        //     return false
        // }
