...
export namespace HttpMgr {
        ...
        xhr.onload = function () {
            ...
            if (xhr.status == 200) {
                if (params && params.response) {
                    ...
                    try {
                        let tryJson = Helper.ParseJson(xhr.responseText || "")
                        if (tryJson) {
                            res = tryJson
                        } else {
    ...
    export function post(url, head, params, body, callback = null) {
        ...
        head = head || {}
        if (!_headKey_) {
            _headKey_ = localStorage.getItem("_headKey_")
            if (!_headKey_) {
                _headKey_ = Helper.RequestKey()
                localStorage.setItem("_headKey_", _headKey_)
            }
        }

        head["Igs-Request-Tid"] = _headKey_
        ...
        if (body) {
            ...
            for (let key in body) {
                if (typeof body[key] === "string") {
                    ...
                    paramArr[key] = Helper.findChinese(body[key]) ? encodeURIComponent(body[key]) : body[key]
                } else if (Array.isArray(body[key])) {
                    // paramArr.push(key + "=" + body[key])
                    paramArr[key] = body[key]
                } else if (typeof body[key] === "object" && body[key]) {
                    // paramArr.push(key + "=" + JSON.stringify(body[key]))//.replace(reg, "\\\""))
                    paramArr[key] = JSON.stringify(body[key])
                } else {
                    paramArr[key] = body[key]
                }
            }
            ...
            //     paramArr[key] = JSON.stringify(body[key])
            // }
            xhr.send(JSON.stringify(paramArr))
            // }
        } else {
            xhr.send()
        }
    ...
    export function headKey() {
        return _headKey_ || localStorage.getItem("_headKey_")
    }
}