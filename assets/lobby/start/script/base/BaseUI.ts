...
export default class BaseUI extends cc.Component {
    ...
    setLabelInfo(name: string | cc.Node, parent: cc.Node | any, info?: any) {
        ...
        if (node) {
            ...
            if (info.font && typeof info.font === "string") {
                let bundle = DataMgr.data.Bundle
                if (bundle) {
                    bundle.load(info.font, cc.Font, (err, res) => {
                        if (err) {
                            return
                        }
    ...
    setSpriteFrame(name: string | cc.Node, parent?: cc.Node | cc.SpriteFrame | string, sprite: cc.SpriteFrame | string | boolean = null, isFixSize: boolean = null, keepSize: boolean = false) {
            ...
            if (-1 !== sprite.indexOf("http://") || -1 !== sprite.indexOf("https://")) {
                ...
                let load = (bName) => {
                    return new Promise((rlv, rjt) => {
                        let bundle = cc.assetManager.getBundle(bName)
                        if(bundle){
                            bundle.load(fName, cc.SpriteFrame, (err, res: cc.SpriteFrame) => {
                                if (err) {
                                    rjt(err)
                                    return
                                }
    ...
    clearButtonClick(name: string | cc.Node, parent?: cc.Node){
        let node = this.getNode(name, parent instanceof cc.Node ? parent : null)
        if (node) {
            let button = node.getComponent(cc.Button)
            if (!button) {
