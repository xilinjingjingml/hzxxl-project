...
export namespace EventMgr {
    ...
    export function offByTag(target: any) {
        ...
        for (let eventName in _handles) {
            _handles[eventName] = _handles[eventName].filter(item => item && item.target !== target)
        }
    }
    ...
    export function clearEvent(eventName: string) {
        if (_handles[eventName]) {
            _handles[eventName] = []
        }
    }
    ...
    export function dispatchEvent(eventName: string, ...args:any): boolean {
        ...
        for (let i = 0; i < len; i++) {
            ...
            event.func.apply(event.target || window, args)
            if (event.once) {
                list[i] = null
            }
        }
