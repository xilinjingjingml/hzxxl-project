...
export namespace AccountSrv {
    ...
    function _loginToken() {
        return new Promise((rlv, rjt) => {
            ...
            HttpMgr.post(url, null, null, loginBody, (res) => {
                ...
                Helper.reportEvent("_loginToken login err=", res.err)
    ...
    function _loginReq(data: any) {
        console.log("_loginReq data", data)
        console.log("_loginReq data.auth_type", data.auth_type)
        console.log("_loginReq parseInt(data.auth_type)", parseInt(data.auth_type))
        let loginBody = {
        ...
        loginBody.auth_info.metadata = loginBody.auth_info.metadata || {}
        loginBody.auth_info.metadata = typeof loginBody.auth_info.metadata === "string" ? loginBody.auth_info.metadata : JSON.stringify(loginBody.auth_info.metadata),
        ...
        console.log("_loginReq loginBody", loginBody)
        let url = "https://" + DataMgr.data.Config.hostname + "/" + LOGIN_URL
        HttpMgr.post(url, null, null, loginBody, (res) => {
    ...
    function _loginErrMsg(msg, callback) {
        ...
        //             confirm: () => { cc.game.end() },
        //         }
        //         UIMgr.OpenUI(cc.assetManager.getBundle("lobbyMain"), "prefab/LoginPop", { param: param })
        //     }  else {
        //         let param = {
        //             msg: msg,
        //             confirmName: "退出",
    ...
    function onLoginSucc(data) {
        ...
        console.log("onLoginSucc account.openid = ", account.openid )
        DataMgr.setData(Helper.GetTokenDataKey(), account, true)
        ...
        try{
            ...
            if(User.Data != null){
                // EventMgr.dispatchEvent(Constants.EVENT_DEFINE.RECORD_POINT, { moduleName: "登录", action: "初始化用户", label: "initUserData" })
                UserSrv.GetUserInfo()
                // EventMgr.dispatchEvent(Constants.EVENT_DEFINE.RECORD_POINT, { moduleName: "登录", action: "初始化用户", label: "GetUserInfo" })

                if (User.Region === "") {            
                    UserSrv.GetPlyDetail(User.OpenID)
                }        
                // EventMgr.dispatchEvent(Constants.EVENT_DEFINE.RECORD_POINT, { moduleName: "登录", action: "初始化用户", label: "GetPlyDetail" })
                EventMgr.dispatchEvent(Constants.EVENT_DEFINE.LOGIN_SUCCESS)                    
            }else{
                // EventMgr.dispatchEvent(Constants.EVENT_DEFINE.RECORD_POINT, { moduleName: "登录", action: "初始化用户", label: "User.Data is null"})
            }
        } catch (error) {
            // EventMgr.dispatchEvent(Constants.EVENT_DEFINE.RECORD_POINT, { moduleName: "登录", action: "初始化用户", label: "onLoginSucc-error="})
        }
    }
    ...
    function initUserData(data): IPlayerData {
        try{
            let items = {}
            // EventMgr.dispatchEvent(Constants.EVENT_DEFINE.RECORD_POINT, { moduleName: "登录", action: "初始化用户", label: "initUserData 1="})
            if (data && data.item_list && data.item_list.length > 0) {
                for (let i of data.item_list) {
                    if(i){
                        ...
                        // EventMgr.dispatchEvent(Constants.EVENT_DEFINE.RECORD_POINT, { moduleName: "登录", action: "初始化用户", label: "initUserData 4="})
                    }
                }
            }
            // EventMgr.dispatchEvent(Constants.EVENT_DEFINE.RECORD_POINT, { moduleName: "登录", action: "初始化用户", label: "initUserData 2="})

            data.reg_time = data.reg_time || 0
            let userData = {
            ...
            // EventMgr.dispatchEvent(Constants.EVENT_DEFINE.RECORD_POINT, { moduleName: "登录", action: "初始化用户", label: "initUserData 3="})
            return userData
        } catch (error) {
            // EventMgr.dispatchEvent(Constants.EVENT_DEFINE.RECORD_POINT, { moduleName: "登录", action: "初始化用户", label: "initUserData-error="})
            return null
        }
    }
    ...
    export function _loginGuest(authInfo: any, callback?: Function) {
        ...
        param["auth_type"] = Constants.AUTH_TYPE.GUEST_LOGIN
        param["token"] = token.token
        param["auth_method"] = "AccountLogin"
        param["auth_info"] = {
            plat_aid: DataMgr.data.Config.pn,
