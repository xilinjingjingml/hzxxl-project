...
export namespace AdSrv {
    ...
    export function initPlatform(ads) {
        if (ads) {
            ...
            for (let i in ads) {
                let v = ads[i]
                if (i === "bannerList") {
                    if(ads[i] instanceof Array){
                        for(let vv of ads[i]){
                            let adId = vv.adId
                            if(Player.GetSelf().newbie && vv.adId_new && vv.adId_new.length > 0){
                                adId = vv.adId_new
                            }
                            PluginMgr.initAds({
                                adPlugin: "",
                        ...
                        let adId = v.adId
                        if(Player.GetSelf().newbie && v.adId_new && v.adId_new.length > 0){
                            adId = v.adId_new
                        }
                        PluginMgr.initAds({
                            adPlugin: "",
    ...
    export function completeAdOrder(callback?: Function) {
        Helper.reportEvent("广告点", "播放广告", "完成订单"+_ad_aid)
        let param = {
            order_no: order_no
        }
    ...
    export function startAdOrder(ad_aid: number, ad_param: string, callback?: Function) {
        AdSrv.createAdOrder(ad_aid, ad_param, (res: IPlayAdCallBack) => {
            if (res && res.order_no && res.order_no.length > 0) {
                AdSrv.completeAdOrder((res) => {
                    if (res && res.code == "00000") {
                        callback && callback({ret:1})
                    }else{
                        callback && callback({ret:2})
                    }
                })
            }else{
                callback && callback({ret:2})
    ...
    function checkShowAds(ad_aid: number, callback?: Function) {
        ...
        if (!ad_config) {
            ...
            if (Player.GetSelf().newbie && ads.adId_new && ads.adId_new.length > 0) {
