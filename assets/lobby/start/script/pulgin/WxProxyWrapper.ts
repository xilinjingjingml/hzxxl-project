...
export namespace WxProxyWrapper {
    ...
    let mShareConfig: any = {
        titles: ["麻溜的，一起来玩红中血流麻将吧！"],
        images: ["https://download.mcbeam.cc/Image/xlhz_share.jpg"]
    }
    let advertVideo = {}
    let advertBanner: {[index: string]: { instance: any, error: any, adUnitId: string }} = {}
    ...
    export function navigateToMiniGame(appId: string, extraData: any) {
        if (mWxValid) {
    ...
    export function showUserInfoButton(node:cc.Node, name, callback?: Function, loadCb?: Function) {
        if (mWxValid && wx.createUserInfoButton) {
            checkUserScope("userInfo", function (canUse) {
                if (!canUse) {
                    if (!mUserInfoButton || !mUserInfoButton[name]) {
                        if (node) {
                            //手机分辨率坐标系，计算node距离中心点距离
                            let pos = node.convertToWorldSpaceAR(cc.Vec2.ZERO)
                            let centerPos = cc.v2(cc.winSize.width/2, cc.winSize.height/2)                            
                            let distanceX = centerPos.x - pos.x
                            let distanceY = centerPos.y - pos.y

                            //计算缩放
                            let screenSzie = cc.view.getFrameSize()
                            let designSize = cc.view.getDesignResolutionSize()
                            // let ratio = screenSzie.width / designSize.width    //宽度适配选择
                            let ratio = screenSzie.height / designSize.height    //高度适配选择                            
                            let width = node.width * ratio
                            let height = node.height * ratio                            
                            
                            //根据缩放计算实际位置（以中心点为参考点）
                            let left = (screenSzie.width/2-distanceX * ratio) - width/2
                            let top = (screenSzie.height/2+distanceY * ratio) - height/2
    ...
    export function hideUserInfoButton(name: string = "") {
        if (name && mWxValid && mUserInfoButton && mUserInfoButton[name]) {
            mUserInfoButton[name].hide()
            return
        }

        if (name.length == 0 && mWxValid && mUserInfoButton) {
    ...
    //todo jin code
    export function payOrderNew(boxItem, callback) {
        let boxParam = {
            goods_param : boxItem.box_gid,
            goods_name : boxItem.name,
            price : boxItem.price,
            store_id : 0,
        ...
        Helper.PostHttp("api/mcbeam-pay-api/payApi/payOrder", null, boxParam, (res, event) => {   
            console.log("payOrder1",res)
            if (res && res.errcode == 0) {
                if (res.bill_no) {
                    // 用米大师余额扣款成功
                    ...
                    // 客户端调米大师支付
                    requestMidasPayment(boxParam, callback, res)
                }
            } else {
                callback({ code: res.Code, msg: res.Detail })
            }
    ...
    //虚拟支付
    // function requestMidasPayment(payInfo, callback, result) {
    ...
    //             Helper.PostHttp("api/mcbeam-pay-api/payApi/payOrder", null, payInfo, (res, event) => {   
    //                 console.log("payOrder2", res)
    //                 if (res && res.errcode == 0) {
    //                     // 用米大师余额扣款成功
    //                     callback({ code: 0, msg : '' })
    //                 } else {
    //                     callback({ code: res.Code, msg: "充值失败" })
    //                 }
    //             })
    //         },
    //         fail: function (res) {
    //             console.log("requestMidasPayment_FAIL", res)
    //             callback({ res:res.errCode, msg:"充值失败" })
    //         },
    //         complete:function (res) {
    //             console.log("requestMidasPayment_complete", res)
    //         }
    //     })
    // }
	...
	*/
    ...
    export function initBannerAd(adUnitId: string) {
        console.log("initBannerAd", adUnitId)
        advertBanner[adUnitId] = advertBanner[adUnitId] || { instance: null, error: null, adUnitId: "" }
        let data = advertBanner[adUnitId]
        data.adUnitId = adUnitId
        if (mWxValid) {
            if (!data.instance && !data.error && wx.createBannerAd) {
                let screenSzie = cc.view.getFrameSize()
                ...
                data.instance.onResize((res) => {
                    ...
                    DataMgr.setData(Constants.DATA_DEFINE.BANNER_HEIGHT, height/cc.view.getFrameSize().height*cc.winSize.height, true)
    ...
    export function showBannerAd(adInfo: any) {
        console.log("showBannerAd adInfo", adInfo)
        if (mWxValid) {
            if(adInfo.createNew && advertBanner[adInfo.adId] && advertBanner[adInfo.adId].instance && !advertBanner[adInfo.adId].error){
                advertBanner[adInfo.adId].instance.destroy()
                advertBanner[adInfo.adId].instance = null
            }
            initBannerAd(adInfo.adId)
            mBannerAdData = advertBanner[adInfo.adId]
            console.log("showBannerAd mBannerAdData", mBannerAdData)
            let data = mBannerAdData
            if (data && data.instance && !data.error) {
                data.instance.show()
                    .then(() => {
    ...
    function onWxEvent() {
        wx.onShow((res) => {
            ...
            EventMgr.dispatchEvent(Constants.EVENT_DEFINE.PLUGIN_ON_SHOW, res)
        })
    }
