GameGlobal.igsgame = GameGlobal.igsgame || wx
let runtimeid = null
! function () {
    function e() {
    ...
    let uid = "";
    let expt_quick_start = wx.getExptInfoSync ? wx.getExptInfoSync() : null;
    function f(e, n) {
        if (n === "hide" || n === "show" || n === "init") {
            e['g0'] = newbie
        }
        if (n === "hide") {
            if (!isFirstHide) {
            ...
            if (lastReport.data) {
                for (let i in lastReport.data) {
                    e[i] = lastReport.data[i]
                    if (i === "tp") {
                        e[i] = e[i] + "-interval" + (Date.now() - lastReport.time)
                    }
                }
            }
        }
        ...
        if (uid == "") {
            try {
                let data = wx.getStorageSync("iGaoShouData6")
                if (data) {
        ...
        w.useOpen && t(), O++, e.as = D, e.at = M, e.rq_c = O, e.ifo = _, e.ak = w.app_key, e.uu = x, e.v = m, e.st = Date.now(), e.ev = n, e.te = S, e.wsr = j, "" !== p(e.ufo) && (e.ufo = e.ufo), e.ec = N, w.useOpen ? "" === b ? K.push(r) : (wx.Queue.push(r), wx.igsReportQueue.push(r1), K.concat()) : (wx.Queue.push(r), wx.igsReportQueue.push(r1))
    }
    ...
    var m = "unknown",
        ...
        try {
			let mv = wx.getAccountInfoSync().miniProgram
            if (mv.version) {
                m = mv.version
            } else if (mv.envVersion) {
                m = mv.envVersion
            }
		} catch (e) {}
...
wx['igsChannel'] = require("./channel")
console.log("igs channel", wx.igsChannel)
...
var newbie = function() {
    return wx.getStorageSync("iGaoShouData5") ? "newbie-false" : "newbie-true"
}()
console.log("report uuid", runtimeid)

var lastReport = {data: null, time: 0};
var isFirstHide = false

let oldDate = Date
wx.igsEvent = {}
wx.igsEvent.report = function () {
    let globalActionId = 1;
    return function(msg, ext, interval = -1) {//interval >0 将被记录到dr字段中
        ...
        for (let i=0;i<ext.length;i++) {
            let uploadToPlatform = null
            try {
                uploadToPlatform = JSON.parse(ext[i])
                let key = uploadToPlatform['k']
                if (key && key.indexOf("wx_performance") !== -1) {
                    let v = uploadToPlatform['v']
                    let pid = v['k']
                    let time = v['v']
                    if (wx.canIUse('reportPerformance')) {
                        console.log("=report performance=", pid, time)
                        wx.reportPerformance(pid, time)
                    }
                }
            } catch (e) {}
            if (uploadToPlatform == null) {
                obj["g"+i] = "" + ext[i]
            }
        }
        obj['sid'] = "" + globalActionId
        if (typeof(interval) === "number" && interval >= 0) {
            // 此字段只透传给dr，不上报
            obj['__interval__'] = interval
        }
        console.log("=EventTrack=", msg, dateFormat("YYYY-MM-dd HH:mm:ss.S", new oldDate()))

        lastReport.data = obj;
        lastReport.time = Date.now()
...
wx.igsEvent.reportUserAction = function() {
    // let globalActionId = 1;
    return function(msg, ext) {
        // if (msg == "" || typeof msg !== "string") {
        //     return
        // }
        // let obj = {tp:msg}
        ...
        //     obj["g"+i] = "" + ext[i]
        // }
        // // msg = ("gid-" + globalActionId + "-") + msg
        // obj['sid'] = "" + globalActionId
        // console.log("=UserActionTrack=", msg, dateFormat("YYYY-MM-dd HH:mm:ss.S", new oldDate()))
        // wx.aldSendEvent(obj);
        // globalActionId++
    }
}()