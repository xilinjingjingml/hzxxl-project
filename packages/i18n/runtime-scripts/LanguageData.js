...
module.exports = {
    ...
    updateSceneRenderers () { // very costly iterations
        ...
        // walk all nodes with localize label and update
        let allLocalizedEditboxs = [];
        for (let i = 0; i < rootNodes.length; ++i) {
            let labels = rootNodes[i].getComponentsInChildren('LocalizedEditbox');
            Array.prototype.push.apply(allLocalizedEditboxs, labels);
        }
        for (let i = 0; i < allLocalizedEditboxs.length; ++i) {
            let label = allLocalizedEditboxs[i];
            if(!label.node.active)continue;
            label.updateLabel();
        }
