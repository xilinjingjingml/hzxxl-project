...
cc.Class({
    ...
    properties: {
        ...
        extraArg: {
            get () {
                return this._extraArg;
            },
            set (val) {
                if (this._extraArg !== val) {
                    this._extraArg = val;
                    if (CC_EDITOR) {
                        this._debouncedUpdateLabel();
                    } else {
    ...
    updateLabel () {
        ...
        if (this.extraArg === "") {
            ...
            try {
                opts = JSON.parse(this.extraArg)
            } catch (error) {
                cc.error("Failed to parse json",this.extraArg)
            }
        }

        let localizedString = i18n.t(this.dataID, opts);
        if (localizedString) {
